...
#### Вторая ловушка-возврат каретки/конец линии

Обмен текстовыми файлами между Linux и Windows всегда создает еще одну проблему. В системах Unix символ _new line_ обозначает конец строки. В DOS/Windows символы _return_ и _new line_ всегда используются вместе, чтобы обозначить её. В наших тестах с Windows Server 2008 эта проблема, казалось, была решена.

Таким образом, текстовые файлы, скопированные из системы DOS в систему Unix, всегда имеют лишние символы в конце строк, а файлы, скопированные другим способом, всегда пропускают поля строк. Поскольку эта проблема очень распространена, сообщество Linux разработало утилиты _**dos2unix**_ и _**unix2dos**_. `Dos2unix` преобразует текстовые файлы из формата DOS/Windows в правильный формат Unix, а `unix2dos` делает это наоборот.

В нашем примере мы должны преобразовать как файл ключа, так и файл конфигурации в формат Unix.
...
Если ваш sample.ovpn выглядит так (как показывает мой `vi`), то вы скопировали файл из Windows в Unix. Чтобы преобразовать его в формат Unix, просто введите следующую команду:

```
debian01:~# dos2unix sample.ovpn
```

Взгляните на этот файл еще раз. Теперь все должно быть в порядке. Повторите этот шаг для файла ключа (`key.txt`). Если вы забудете этот шаг, OpenVPN найдет разные ключи в обеих системах и, следовательно, запретит установку туннеля.

---

В `dos2unix` содержится в пакете модуля `sysutils` в системах Debian. Запустите `apt-get install sysutils`, чтобы установить эти инструменты.

---

### Настройка системы Linux

На следующем этапе мы должны адаптировать конфигурацию Linux, так же, как мы делали это раньше в системах Windows. Мы будем использовать точно такую же конфигурацию, как и в нашем первом примере. Нужно изменить только три строки. На следующем рисунке представлен обзор того, как будут настраиваться интерфейсы:

![](/pics/pic7-18.png)

Конфигурация Linux OpenVPN так же проста, как и его аналог Windows. Просто измените следующие строки в своем `sample.ovpn`:
* `remote 10.10.10.103`
* `ifconfig 10.3.0.5 255.255.255.0`
* `secret key.txt`

После изменения ранее упомянутых строк, адаптируйте их к вашим потребностям. IP-адрес, указанный в строке `remote 10.10.10.103`, должен быть заменен на адрес вашего сервера Windows. IP-адрес, указанный в строке `ifconfig 10.3.0.5 255.255.255.0`, определяет IP-адрес сетевого интерфейса виртуального туннеля. Возможно, вы заметили, что этот IP-адрес может быть выбран свободно в этом сегменте сети.

Затем запустите туннель, введя следующую команду в каталоге конфигурации:

```
openvpn –-config sample.ovpn
```

Эта команда является основной частью OpenVPN. Независимо от того, какую операционную систему вы используете, это способ запуска туннелей для целей тестирования, а также вызывамый скриптами, предоставляющими услуги OpenVPN.
...
Wed Oct 19 00:23:01 2005 us=334726 Local Options String: 'V4,devtype tap,link-mtu 1577,tun-mtu 1532,proto UDPv4,ifconfig 10.3.0.0
...
Эта программа также является частью установки Windows. Запустите командную строку и перейдите в каталог, содержащий файл конфигурации. Введите команду `openvpn --config sample.ovpn` и нажмите _Enter_. Вы получите результат, показанный на следующем скриншоте. Как можете видеть, поведение OpenVPN почти идентично версии Linux.

К сожалению, если вы запускаете туннель вручную, то **OpenVPN GUI** не сможет его заметить.
